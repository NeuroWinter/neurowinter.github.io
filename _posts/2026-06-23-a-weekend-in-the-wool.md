...
- **A protection layer** (protecting the scripts) — the bit that explains why I
  couldn't read qlk in the first place. One operator, wyourname, runs what is
  basically DRM as a service: encrypted `.so` loaders plus a C2 server that
  hands out the decryption key per machine. The other authors rent it so their
  scripts can't just be lifted straight off GitHub. At least this is what I
  think is happening.
...
The whole thing is really just one pipeline: encrypted scripts -> Qinglong runs
